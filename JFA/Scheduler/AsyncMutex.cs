...
namespace JFA.Scheduler
{
    ...
    /// </remarks>
    internal sealed class AsyncMutexPlzFix
    {
        public AsyncMutexPlzFix(int maxCount)
        {
            this._gate = this._maxCount;
        ...
        public ValueTask EnterAsync(CancellationToken cancellationToken)
        {
            ...
            return
                ...
                Interlocked.Decrement(ref _gate) >= 0 ? ValueTask.CompletedTask :
        ...
        public void Exit(short token)
        {
            ...
            void Contended()
            {
                ...
                if (w is not null)
                {
                    //TODO: FIX!!!!!
                    w.CancellationRegistration.Unregister();
                    w.TrySetResult(token);
                }
            }
        }

        /// <summary>Represents a waiter for the mutex.</summary>
        private sealed class Waiter : IThreadPoolWorkItem,IValueTaskSource
        {
            private ManualResetValueTaskSourceCore<int> SourceCore =
                new ManualResetValueTaskSourceCore<int>();

            private int canReuse = 1;
            public Waiter(AsyncMutexPlzFix owner)
            {
                Owner = owner;
            }

            public AsyncMutexPlzFix Owner { get; }
            ...
            public ValueTask AsTask(short token)
            {
            ...
            public void TrySetResult(short version)
            {
                if (version == SourceCore.Version)
                {
                    SourceCore.SetResult(0);
                }
            }

            public void TrySetCanceled(CancellationToken token)
            {
                if (Interlocked.Decrement(ref canReuse) == 0)
                {
                    SourceCore.SetException(
                        new OperationCanceledException(token));
                }
            }
            
            public short Ver => SourceCore.Version;
