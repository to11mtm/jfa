...
namespace JFA.Channels.Pool;

public sealed class SloppyPool<T>
{
    readonly int _max;
    int curr;
    ConcurrentQueue<T> nodes = new ConcurrentQueue<T>();
    public SloppyPool(int max)
    {
        _max = max;
    }
    ...
    public bool TryGet(out T item)
    {
        ...
        {
            return nodes.TryDequeue(out item);
        }
    }

    public bool TryAdd(T item)
    {
        while (Volatile.Read(ref curr) < _max)
        {
            var nv = Interlocked.Increment(ref curr);
            if (nv < _max)
            {
                nodes.Enqueue(item);
                return true;
